...
function maxCheckOutDate(from) {
  return addDays(stripTime(from || defaultCheckIn()), MAX_STAY_NIGHTS);
}

function nightsBetweenDates(checkIn, checkOut) {
  const ci = stripTime(checkIn).getTime();
  const co = stripTime(checkOut).getTime();
...
export default function BookingsScreen() {
  ...
  const applyPickedDate = (target, raw) => {
    ...
    if (flow === 'create') {
      if (target === 'checkIn') {
        ...
        const maxOut = maxCheckOutDate(d);
        setCheckInDate(d);
        setCheckOutDate((prev) => {
          const p = stripTime(prev);
          if (p.getTime() <= d.getTime()) return addDays(d, 1);
          if (p.getTime() > maxOut.getTime()) return maxOut;
          return p;
        });
      } else {
        const maxOut = maxCheckOutDate(checkInDate);
        ...
        if (d.getTime() > maxOut.getTime()) {
          Alert.alert('Invalid stay', 'Maximum stay is 60 nights. Choose an earlier check-out date.');
          return;
        }
        setCheckOutDate(d);
      }
      ...
      if (target === 'checkIn') {
        ...
        const maxOut = maxCheckOutDate(editCheckInDate);
      ...
      <View style={styles.iosInlinePickerRoot} pointerEvents="box-none">
        ...
        <View style={styles.iosPickerSheet}>
          ...
          <DateTimePicker
            ...
            display="spinner"
            themeVariant="light"
            minimumDate={dateTarget === 'checkOut' ? addDays(activeCheckIn, 1) : defaultCheckIn()}
            maximumDate={dateTarget === 'checkOut' ? maxCheckOutDate(activeCheckIn) : undefined}
            onChange={(_, d) => {
  ...
  const promptGuestLogin = (closeModal = false) => {
    ...
    Alert.alert('Sign in first', 'Please sign in first to reserve a room.', [
  ...
  const submitCancelRequest = async (overrideId) => {
    const target = overrideId || cancelTargetId;
    if (!target || !customerToken) return;
    try {
      setAuthToken('customer', customerToken);
      await api.post(`/customer-auth/bookings/${target}/cancel`);
      Alert.alert('Deleted', 'Reservation deleted.');
      setCancelTargetId(null);
      fetchData();
    } catch (error) {
      Alert.alert('Error', error.response?.data?.error || 'Could not delete reservation');
    }
  };
  ...
  const handleAddBooking = async () => {
    ...
    if (stripTime(checkInDate).getTime() < defaultCheckIn().getTime()) {
      Alert.alert('Error', 'Check-in cannot be in the past');
      return;
    }
    if (checkOut <= checkIn) {
      Alert.alert('Error', 'Check-out must be after check-in');
    ...
    if (nightsBetweenDates(checkInDate, checkOutDate) > MAX_STAY_NIGHTS) {
      Alert.alert('Error', 'Maximum stay is 60 nights. Choose an earlier check-out date.');
      return;
    }
  ...
  const renderItem = ({ item }) => {
    ...
    return (
      <View style={styles.card}>
        ...
        <Text style={styles.cardText}>Payment Method: {item.paymentMethod || 'Card'}</Text>
        ...
        <View style={styles.cardActions}>
          ...
          {canCancel ? (
            <TouchableOpacity
              ...
              onPress={() => {
                const bId = normalizeBookingMongoId(item._id ?? item.id);
                if (Platform.OS === 'web' && typeof window !== 'undefined' && typeof window.confirm === 'function') {
                  if (window.confirm('Delete this reservation permanently?')) {
                    setCancelTargetId(bId);
                    submitCancelRequest(bId);
                  }
                } else {
                  Alert.alert('Delete reservation', 'Delete this reservation permanently?', [
                    { text: 'Keep', style: 'cancel' },
                    { text: 'Delete', style: 'destructive', onPress: () => { setCancelTargetId(bId); submitCancelRequest(bId); } },
                  ]);
                }
              }}
            >
              <Text style={styles.cancelReqBtnText}>Delete reservation</Text>
            </TouchableOpacity>
          ) : null}
        </View>
      </View>
  ...
  const renderDateRow = (flow, target, label) => {
    ...
    const today = defaultCheckIn();
    const minIn = toYmd(today);
    const maxOut = toYmd(maxCheckOutDate(ci));
        ...
        <>
          ...
          <View style={styles.dateWebShell}>
            {createElement('input', {
              type: 'date',
              value: toYmd(value),
              min: target === 'checkOut' ? minOut : minIn,
              max: target === 'checkOut' ? maxOut : undefined,
              onChange: (e) => {
                ...
                if (target === 'checkIn') {
                  ...
                  const maxOutDate = maxCheckOutDate(next);
                  if (flow === 'create') {
                    setCheckInDate(next);
                    setCheckOutDate((prev) =>
                      stripTime(prev).getTime() <= next.getTime()
                        ? addDays(next, 1)
                        : stripTime(prev).getTime() > maxOutDate.getTime()
                          ? maxOutDate
                          : stripTime(prev)
                    );
                  } else {
                  ...
                  Alert.alert('Invalid dates', 'Check-out must be after check-in.');
                } else if (next.getTime() > maxCheckOutDate(ci).getTime()) {
                  Alert.alert('Invalid stay', 'Maximum stay is 60 nights. Choose an earlier check-out date.');
                } else if (flow === 'create') {
            ...
            <View style={styles.modalContent}>
              ...
              >
                ...
                <View style={{ flexDirection: 'row', gap: 10, marginBottom: 12 }}>
                  {['Card', 'Cash', 'Bank Transfer'].map((pm) => {
                    const isSelected = newPaymentMethod === pm;
                    return (
                      <TouchableOpacity
                        key={pm}
                        style={{
                          ...
                          borderWidth: 1,
                          borderColor: isSelected ? '#3d2b1f' : '#e8e0d5',
                          borderRadius: 8,
                          backgroundColor: isSelected ? '#f5f0e8' : '#fff'
                        }}
                        onPress={() => setNewPaymentMethod(pm)}
                      >
                        <Text style={{
                          color: isSelected ? '#3d2b1f' : '#6b6b6b',
              ...
              >
                ...
                {editingBooking ? (
                  <>
                    ...
                    {editingBooking.specialRequests ? <Text style={styles.cardNote}>Notes: {editingBooking.specialRequests}</Text> : null}
                    <Text style={styles.cardNote}>Payment Method: {editingBooking.paymentMethod || 'Not specified'}</Text>
