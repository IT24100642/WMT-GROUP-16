...
export default function CustomerProfileScreen({ navigation }) {
  ...
  const onDeleteAccount = useCallback(() => {
    const runDelete = async () => {
      try {
        ...
        setAuthToken('customer', customerToken);
        await api.delete('/customer-auth/me');
        doSignOut();
        if (Platform.OS === 'web' && typeof window !== 'undefined') {
          window.alert('Your guest account has been deleted.');
        } else {
          Alert.alert('Account deleted', 'Your guest account has been deleted.');
        }
      } catch (e) {
    ...
    if (Platform.OS === 'web' && typeof window !== 'undefined' && typeof window.confirm === 'function') {
      if (window.confirm('Delete your guest account permanently? This cannot be undone.')) runDelete();
      return;
    }

    Alert.alert('Delete account', 'Delete your guest account permanently? This cannot be undone.', [
      { text: 'Cancel', style: 'cancel' },
      { text: 'Delete', style: 'destructive', onPress: runDelete },
    ]);
  }, [customerToken, doSignOut]);
  ...
  const saveProfile = useCallback(async () => {
    try {
      ...
      const nextEmail = String(editEmail || '').trim().toLowerCase();
      ...
      setAuthToken('customer', customerToken);
      await api.patch('/customer-auth/me', {
        name: nextName,
        email: nextEmail,
  ...
  }, [customerToken, editName, editEmail, editPhone, refresh, closeProfileEditor]);
  ...
  return (
    <ScrollView style={styles.root} contentContainerStyle={styles.content}>
      ...
      <View style={styles.card}>
        ...
        <TouchableOpacity style={styles.inlineActionBtn} onPress={openProfileEditor}>
          <Text style={styles.inlineActionText}>Update profile</Text>
        </TouchableOpacity>
      </View>
      ...
      <View style={styles.card}>
        ...
        {bookingsLoading ? (
          ...
          normalizedBookings.map((b) => {
            ...
            const canMutate = !isCancelled && !b.checkedInAt;
            return (
      ...
      <Modal visible={profileModalVisible} transparent animationType="fade" onRequestClose={closeProfileEditor}>
        <View style={styles.modalOverlay}>
          <View style={styles.modalCard}>
            <Text style={styles.modalTitle}>Update Profile</Text>
            <Text style={styles.modalSub}>Edit your guest account details.</Text>
            <TextInput
              style={styles.input}
            ...
            />
            <View style={styles.modalActions}>
              <TouchableOpacity style={styles.modalGhostBtn} onPress={closeProfileEditor} disabled={savingProfile}>
                <Text style={styles.modalGhostText}>Cancel</Text>
              </TouchableOpacity>
              <TouchableOpacity
...
const styles = StyleSheet.create({
  ...
  deleteAccountBtn: { alignItems: 'center', paddingBottom: 8 },
  deleteAccountText: { color: '#7f1d1d', fontWeight: '700', fontSize: 14, textDecorationLine: 'underline' },
  inlineActionBtn: { marginTop: 8, alignSelf: 'flex-start', paddingVertical: 8, paddingHorizontal: 12, borderRadius: 8, borderWidth: 1, borderColor: C.darkBrown },
  inlineActionText: { color: C.darkBrown, fontWeight: '700' },
  modalOverlay: {
    flex: 1,
