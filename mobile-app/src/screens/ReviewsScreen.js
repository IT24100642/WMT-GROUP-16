...
export default function ReviewsScreen() {
  ...
  const openEditModal = (item) => {
    setReviewName(String(item.reviewerName || '').trim());
    setReviewRating(Number(item.rating) || 0);
    setReviewText(String(item.text || item.comment || '').trim());
  ...
  const submitReview = async () => {
    ...
    try {
      ...
      if (editingReviewId) {
        await api.patch(`/customer-auth/reviews/${editingReviewId}`, {
          text,
          rating: reviewRating,
          reviewerName: name,
        });
  ...
  const deleteMine = (item) => {
    ...
    if (Platform.OS === 'web') {
      if (window.confirm('Hide this review from guests?')) {
        executeDelete();
      }
    } else {
  ...
  const renderItem = ({ item }) => {
    const rawCid = item.customerId?._id || item.customerId?.id || item.customerId || item.customer;
    const cid = rawCid != null ? String(rawCid) : '';
    const isMine = Boolean(customerId && cid === customerId);
    
    // Debug log to help identify if the user is correctly recognized
    if (customerId) {
      console.log(`[ReviewsScreen] Review ID: ${item._id}, Reviewer CID: ${cid}, My ID: ${customerId}, isMine: ${isMine}`);
    }
        ...
        {isMine ? (
          <View style={{ flexDirection: 'row', justifyContent: 'flex-end', gap: 12, marginTop: 12 }}>
            <TouchableOpacity onPress={() => openEditModal(item)}>
              <Text style={{ color: '#c9a96e', fontWeight: 'bold' }}>Edit my review</Text>
            </TouchableOpacity>
            <TouchableOpacity onPress={() => deleteMine(item)}>
        ...
        <KeyboardAvoidingView style={styles.modalOverlay} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
          <ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={{ flexGrow: 1, justifyContent: 'center', padding: 20 }}>
            <View style={styles.modalContent}>
              <Text style={styles.modalTitle}>{editingReviewId ? 'Edit your review' : 'Write a review'}</Text>
              ...
              <View style={styles.modalActions}>
                ...
                <TouchableOpacity style={styles.saveBtn} onPress={submitReview}>
                  <Text style={styles.saveBtnText}>{editingReviewId ? 'Save' : 'Submit'}</Text>
                </TouchableOpacity>
              </View>
            </View>
          </ScrollView>
